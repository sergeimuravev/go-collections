...
func TestAdd(t *testing.T) {
	samples := []struct {
		...
		{New(5, 4, 3, 2, 1), -1, []int{-1, 1, 2, 3, 4, 5}},
		{New(), 0, []int{0}},
	}
...
func TestAddAfter(t *testing.T) {
	...
	for _, sample := range samples {
		count := sample.list.Count()
		nodeAfter := sample.list.Find(sample.valueAfter)
		node := sample.list.AddAfter(nodeAfter, sample.valueToAdd)
...
func TestAddBefore(t *testing.T) {
	samples := []struct {
		...
		{New(1, 2, 3, 4, 5), 0, 0, []int{5, 4, 3, 2, 1, 0}},
		{LinkedList{}, 1, 0, []int{0}},
	}

	for _, sample := range samples {
		count := sample.list.Count()
		nodeBefore := sample.list.Find(sample.valueBefore)
		node := sample.list.AddBefore(nodeBefore, sample.valueToAdd)
